...
class SequentialBatchConv2DLayer(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1,
                 padding=0, dilation=1):
        super(SequentialBatchConv2DLayer, self).__init__()
    ...
    def forward(self, x, weight, bias=None):
        ...
        out = torch.stack([F.conv2d(per_task_x, weight=per_task_weight, bias=None, stride=self.stride, dilation=self.dilation, groups=1,
                       padding=self.padding) for per_task_x, per_task_weight in zip(x, weight)], dim=0)
